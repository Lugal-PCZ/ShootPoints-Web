...
### Added
- Added exporters and importers for Classes/Subclasses and Stations.
- Added updating instructions to the README.

### Changed
- Rearranged the Utilities window, grouping similar functions under distinct headers.
- Updated the README to reflect the UI changes.

### Fixed
- Added dates, which had been accidentally omitted, to the 1.9.x entries in this CHANGELOG.
...
### Changed
- Moved the configs settings from the configs.ini file into the ShootPoints database so that configs can be backed up along with the project database.
- Indicates the existence of an unsaved shot when viewing the details of the current grouping or current session.
- Added an alert dialog to indicate that a database reset has completed.
